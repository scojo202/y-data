  ...
  if(Y_IS_VECTOR(input)) {
    ...
    for (j=0;j<d->sop.length1;j++) {
      for (i=0;i<d->sop.length2;i++) {
        v[i*d->sop.length1+j]=m[(i+d->sop.start2)*ncol+j+d->sop.start1];
      }
    }
  }
  return v;
...
static void
y_subset_operation_class_init (YSubsetOperationClass *subset_klass)
{
  ...
  g_object_class_install_property (gobject_klass, SUBSET_PROP_START1,
		g_param_spec_int ("start1", "Start index #1",
			"First index",
			0, 2000000000, 0,
			G_PARAM_READWRITE));
  ...
  g_object_class_install_property (gobject_klass, SUBSET_PROP_START2,
		g_param_spec_int ("start2", "Start index #2",
			"Second index",
			0, 2000000000, 0,
			G_PARAM_READWRITE));
...
static void
y_subset_operation_init(YSubsetOperation *slice) {
