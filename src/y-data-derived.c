...
static void
y_scalar_derived_interface_init (YDerivedInterface *iface)
{
...
static double
scalar_derived_get_value (YScalar *sca)
{
  YDerivedScalar *scas = (YDerivedScalar *)sca;

  if(sca==NULL) return NAN;

  /* call op */
  YOperationClass *klass = Y_OPERATION_GET_CLASS(scas->der.op);
  if(scas->der.task_data == NULL) {
    scas->der.task_data = y_operation_create_task_data(scas->der.op,scas->der.input);
  }
  else {
    y_operation_update_task_data(scas->der.op,scas->der.task_data,scas->der.input);
  }
...
static void
scalar_op_cb (GObject *source_object,
                        GAsyncResult *res,
                        gpointer user_data)
{
  /* set outputs */
...
static void
scalar_on_input_changed(YData *data, gpointer   user_data) {
  ...
  if(!d->der.autorun) {
    ...
    if(klass->thread_safe) {
      ...
      y_operation_update_task_data(d->der.op,d->der.task_data,data);
      y_operation_run_task(d->der.op,d->der.task_data,scalar_op_cb,d);
    }
    else {
                        ...
                        GParamSpec   *pspec)
{
  ...
  switch (property_id)
    ...
    case PROP_INPUT:
      ...
      g_signal_connect(s->der.input,"changed",G_CALLBACK(scalar_on_input_changed),s);
...
static void
y_vector_derived_interface_init (YDerivedInterface *iface)
{
...
vector_derived_finalize (GObject *obj)
{
	...
	g_object_unref(vec->der.input);
	  ...
	  YOperationClass *klass = (YOperationClass *) G_OBJECT_GET_CLASS(vec->der.op);
...
vector_derived_load_len (YVector *vec)
{
  ...
  YOperationClass *klass = (YOperationClass *) G_OBJECT_GET_CLASS(vecd->der.op);
  g_assert(klass);
...
vector_derived_load_values (YVector *vec)
{
  ...
  if(vecs->der.task_data == NULL) {
    vecs->der.task_data = y_operation_create_task_data(vecs->der.op,vecs->der.input);
  }
  else {
    y_operation_update_task_data(vecs->der.op,vecs->der.task_data,vecs->der.input);
  }
...
on_input_changed_after(YData *data, gpointer   user_data) {
  ...
  if(!d->der.autorun) {
    ...
    if(klass->thread_safe) {
      /* get task data, run in a thread */
      y_operation_update_task_data(d->der.op,d->der.task_data,data);
      y_operation_run_task(d->der.op,d->der.task_data,op_cb,d);
    }
    else {
      /* load new values into the cache */
                        ...
                        GParamSpec   *pspec)
{
  ...
  switch (property_id)
    ...
    case PROP_INPUT:
      ...
      g_signal_connect(d->input,"changed",G_CALLBACK(on_input_changed_after),v);
...
static void
y_derived_matrix_interface_init (YDerivedInterface *iface)
{
...
static void
derived_matrix_finalize (GObject *obj)
{
...
static YMatrixSize
derived_matrix_load_size (YMatrix *vec)
{
...
static double *
derived_matrix_load_values (YMatrix *vec)
{
  ...
  if(vecs->currsize.rows != size.rows || vecs->currsize.columns != size.columns) {
    if(vecs->cache)
      g_free(vecs->cache);
...
static double
derived_matrix_get_value (YMatrix *vec, unsigned i, unsigned j)
{
  YMatrixSize size = y_matrix_get_size(vec);
...
static void
on_input_changed_after2(YData *data, gpointer   user_data) {
  ...
  if(!d->der.autorun) {
    ...
    if(klass->thread_safe) {
      /* get task data, run in a thread */
      y_operation_update_task_data(d->der.op,d->der.task_data,data);
      y_operation_run_task(d->der.op,d->der.task_data,op_cb2,d);
    }
    else {
      /* load new values into the cache */
                        ...
                        GParamSpec   *pspec)
{
  ...
  switch (property_id)
    ...
    case PROP_INPUT:
      d->input = g_value_get_object(value);
      g_signal_connect(d->input,"changed",G_CALLBACK(on_input_changed_after2),v);
      y_data_emit_changed(Y_DATA(v));
      break;
    case PROP_OPERATION:
      d->op = g_value_get_object(value);
...
static void
y_derived_matrix_class_init (YDerivedMatrixClass *slice_klass)
{
  GObjectClass *gobject_class = (GObjectClass *) slice_klass;
  //YDataClass *ydata_klass = (YDataClass *) gobject_class;
...
static void
y_derived_matrix_init(YDerivedMatrix *der) {
  der->der.input = NULL;
  der->cache = NULL;
}
